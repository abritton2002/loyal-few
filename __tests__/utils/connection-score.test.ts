...
describe('calculateConnectionScore', () => {
  ...
  it('handles extremely low emotion ratings', () => {
    ...
    const score = calculateConnectionScore(relationship);
    // Low ratings might still increase the score because there's an interaction,
    // but it should be lower than with a high rating
    const highRatingRelationship = {
      ...baseRelationship,
      emotionHistory: [{ date: new Date().toISOString(), rating: 10 }],
      interactions: [
        {
          id: '1',
          date: new Date().toISOString(),
    ...
    expect(score).toBeLessThan(calculateConnectionScore(highRatingRelationship));
  });
