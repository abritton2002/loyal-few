...
export default function EditRelationshipScreen() {
  ...
  const { getRelationshipById, updateRelationship } = useRelationshipStore();
  ...
  const [avatar, setAvatar] = useState<string | undefined>(relationship.avatar);
  const [selectedTags, setSelectedTags] = useState<RelationshipTag[]>(relationship.tags);
  const [notes, setNotes] = useState(relationship.notes || '');
  const [reminderFrequency, setReminderFrequency] = useState(relationship.reminderFrequency || 7);
